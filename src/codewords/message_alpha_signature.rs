...
impl CWMessageAlphaSignature {
    pub fn new(signature: u32, chars: &[u8]) -> Result<CWMessageAlphaSignature, &'static str> {
        if chars.len() != 2 {
...
mod tests {
    ...
    fn test_message_alpha_signature() {
        ...
        let msg_signature = CWMessageAlphaSignature::new(0x7F, &chars).unwrap();
    ...
    fn test_message_alpha_signature_invalid() {
        let chars = vec![0x42, 0x23];
        assert_eq!(CWMessageAlphaSignature::new(0x800, &chars).is_err(), true);
    }

    #[test]
    fn test_message_alpha_signature_too_few_chars() {
        let chars = vec![0x42];
        assert_eq!(CWMessageAlphaSignature::new(0x7FF, &chars).is_err(), true);
    }
