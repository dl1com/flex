...
impl CWMessageAlphaHeader {
    pub fn new(
        ...
        retrieval_flag: u32,
        mail_drop_flag: u32,
    ) -> Result<CWMessageAlphaHeader, &'static str> {
        if message_continued_flag <= 1 && fragment_number <= 3 && message_number <= 63 &&
            retrieval_flag <= 1 && mail_drop_flag <= 1
        {
...
mod tests {
    ...
    fn test_message_alpha_header_1() {
        let msg_header = CWMessageAlphaHeader::new(0, 3, 63, 1, 0).unwrap();
        assert_eq!(msg_header.get_codeword() & 0x1FFA00, 0x0FF800);
    }

    #[test]
    fn test_message_alpha_header_2() {
        let msg_header = CWMessageAlphaHeader::new(0, 3, 23, 0, 0).unwrap();
